...
pub async fn server(ip: String, config_db: HashMap<String, String>) -> std::io::Result<()> {
    ...
    let server = HttpServer::new(move || {
        App::new()
            ...
            .service(web::resource("/update_open_alarm").route(web::post().to(handlers::update_account_open_alarm)))
            .service(web::resource("/update_threshold").route(web::post().to(handlers::update_account_threshold)))
